...
const decryptChannelRouter = express.Router()

decryptChannelRouter.get('/decrypt/:channelId/:fileName/:key/:iv', async (req, res) => {
    const { channelId, fileName, key, iv } = req.params;
    ...
    try {
      ...
      const filePath = path.join(streamSafePath, 'videos', channelId, fileName);
