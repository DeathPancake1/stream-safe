...
export const channelEncryptAndUpload = async (key, originalFile: File, userData, chatName, setUploadProgress, channelKey) => {
  try {
    ...
    const now = Date.now();
    setUploadProgress
    addChannelVideo('', originalFile.name+'.enc', chatName, now, true, iv, originalFile.type, channelKey)
