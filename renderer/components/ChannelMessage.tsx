import { Box, Button, Typography } from "@mui/material";
import { ChannelVideo, keysDB, updateChannelVideo, updateVideo } from "../indexedDB";
import DownloadIcon from '@mui/icons-material/Download';
import { useUser } from "../providers/UserContext";
import PlayArrowIcon from '@mui/icons-material/PlayArrow';
import { localUrl } from "../config";
import decryptAESHex from "../helpers/decryption/decryptAESHex";
import secureLocalStorage from "react-secure-storage";
import ChannelType from "../types/channel-type";
import writeFileChannel from "../helpers/fileSystem/writeFileChannel";

interface Props {
    incoming: boolean;
    message: ChannelVideo;
    messages: ChannelVideo[];
    channel: ChannelType;
    setPlayVideo: (boolean)=>void;
    setVideo: (videoUrl: string)=>void;
    setMessages: (videos: ChannelVideo[])=>void
}

export default function ChannelMessage({ incoming, message, messages, setPlayVideo, setVideo, setMessages, channel}: Props) {
  const {userData, updateUser} = useUser()
  const backgroundColor = incoming ? '#e0e0e0' : '#333333'; // Adjust colors as needed
  const textColor = incoming ? '#333' : '#fff'; // Adjust text color for better contrast

  const markMessageDownloaded = ()=>{
    const updatedMessages = [...messages];
    const messageIndex = updatedMessages.findIndex((msg) => msg.id === message.id);
    if (messageIndex !== -1) {
      // Update the downloaded property of the specific message
      updatedMessages[messageIndex] = {
        ...updatedMessages[messageIndex],
        downloaded: true,
      };

      // Use setMessages to update the state with the modified array
      setMessages(updatedMessages);
    }

    // Update the message in IndexedDB
    updateChannelVideo(message.id, { ...message, downloaded: true });
  }

  const download = async ()=>{
    const jwt = userData.jwt
    const url = process.env.API_URL
    const apiKey = process.env.API_KEY
    const status = await writeFileChannel(channel.channelId, message.name, jwt, url+'/channelFiles/downloadVideo', apiKey, message.path)
    if(status){
      markMessageDownloaded()
    }
  }

  const playVideo = async ()=>{
    const masterKey = secureLocalStorage.getItem('masterKey').toString()
    const decryptedKey = await decryptAESHex(masterKey, channel.key)
    const base_url = localUrl;
    setVideo(base_url+'/decrypt/'+channel.channelId+'/'+message.name+'/'+decryptedKey+'/'+message.iv)
    setPlayVideo(true)
  }

  return (
    <Box
      sx={{
        width: 'fit-content',
        minWidth: '10%',
        maxWidth: '40%',
        height: 'fit-content',
        backgroundColor: backgroundColor,
        borderRadius: '8px',
        boxShadow: '0 2px 5px rgba(0, 0, 0, 0.1)',
        marginLeft: incoming ? 'initial' : 'auto',
        padding: '10px',
        color: textColor,
        fontWeight: 'bold',
        marginTop: '10px',
        display: "flex",
        flexDirection: 'column'
      }}
    >
      <Box
        sx={{
          margin: '2px',
          textAlign: 'center',
          overflowX: 'hidden'
        }}
      >
        <Typography fontFamily={'roboto'} fontSize={24} >
          {message.name.slice(0, -4)}
        </Typography>
      </Box>
      <Box>
        {
          message.downloaded?
          <Button 
            component="label" 
            color="secondary"
            variant="text" 
            startIcon={<PlayArrowIcon />} 
            onClick={()=>{
              playVideo()
            }} 
            fullWidth
          >
            Play Video
          </Button>
          :
          <Button
            component="label" 
            variant="text" 
            startIcon={<DownloadIcon />} 
            onClick={()=>download()} 
            fullWidth
          >
            Download file
          </Button>
        }
        
      </Box>
    </Box>
  );
}
