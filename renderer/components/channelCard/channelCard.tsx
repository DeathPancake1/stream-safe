...
export default function ChannelCard({ title, imageId, description, ownerId, channelId }) {
    ...
    const [owner, setOwner] = useState<User>({ firstname: "", lastname: "", photoId: 0 });
    ...
    useEffect(() => {
        getUserById({ ownerId: ownerId, jwt: userData.jwt }, {
            onSuccess: (response) => {
    ...
    return (
        <Card sx={{ maxWidth: 345, minWidth: 250, height: "300px", display: "flex", flexDirection: "column" }}>
